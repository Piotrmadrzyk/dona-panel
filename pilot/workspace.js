(function () {
  ...
  function showMailReply(mail){
    ...
    dialog.innerHTML='<form method="dialog" id="mailReplyForm"><div class="system-dialog-head"><div><span>SZKIC ODPOWIEDZI</span><h2>'+escape(mail.title||mail.id)+'</h2><p>Od: '+escape(mail.sender||'nieznany nadawca')+'. DONA przygotuje szkic na podstawie historii wątku, pokaże go w tej rozmowie i wyśle do zatwierdzenia — zobaczysz go jako kartę w Skrzynce decyzji oraz jako prośbę na Telegramie. Nic nie zostanie wysłane, dopóki nie zatwierdzisz.</p></div><button type="button" class="icon-button" data-mail-dialog-close aria-label="Zamknij">×</button></div><div class="system-form-grid"><label class="is-wide"><span>Co ma się znaleźć w odpowiedzi (opcjonalnie)</span><textarea name="notes" maxlength="1500" placeholder="Np. potwierdź termin, zapytaj o cenę, podziękuj i zamknij temat"></textarea></label></div><div class="system-dialog-actions"><button type="button" class="secondary" data-mail-dialog-close>Anuluj</button><button type="submit" class="primary">Przygotuj szkic</button></div></form>';
    dialog.querySelectorAll('[data-mail-dialog-close]').forEach(b=>b.addEventListener('click',()=>dialog.close()));
    dialog.querySelector('form').addEventListener('submit',async e=>{
      e.preventDefault();
      const values=Object.fromEntries(new FormData(e.currentTarget).entries());
      dialog.close();
      const prompt='Przygotuj szkic odpowiedzi na wiadomość o identyfikatorze '+mail.id+' ("'+mail.title+'" od '+(mail.sender||'nieznanego nadawcy')+'), użyj narzędzia poczta z akcją draft.'+(values.notes?' Uwzględnij: '+values.notes+'.':'')+' Szkic ma trafić do zatwierdzenia w Skrzynce decyzji — nie wysyłaj niczego bez mojej zgody.';
      toast('DONA przygotowuje szkic — zobaczysz go w rozmowie.');
