(function () {
  ...
  function openChat(){ $('app').classList.remove('chat-collapsed');$('app').classList.add('chat-open');$('chatToggle').setAttribute('aria-expanded','true'); }
  function closeChat(){ $('app').classList.remove('chat-open');if(innerWidth>1140)$('app').classList.add('chat-collapsed');$('chatToggle').setAttribute('aria-expanded','false');$('chatToggle').focus(); }
  ...
  function discuss(collection,id){const r=rows(collection).find(x=>x.id===id);if(!r)return;draft('Sprawdź '+(collection==='approvals'?'prośbę o zatwierdzenie':collection==='offers'?'ofertę':'sprawę')+' o identyfikatorze '+r.id+' ('+(r.title||r.name||'')+'). Podsumuj aktualny stan i zaproponuj kolejny krok. Najpierw tylko odczyt — bez wysyłek i zmian.');}
  function showDetail(collection,id){
    const r=rows(collection).find(x=>x.id===id);if(!r)return;const dialog=$('detailDialog');dialog.dataset.collection=collection;dialog.dataset.id=id;
    ...
    $('detailActions').replaceChildren();const link=safeUrl(r.url);if(link){const a=el('a','Otwórz dokument ↗','secondary');a.href=link;a.target='_blank';a.rel='noopener noreferrer';$('detailActions').append(a);}if(collection==='approvals'){const edit=el('button','Edytuj','secondary');edit.onclick=()=>{dialog.close();window.DonaCommand.revise('approval',id);};$('detailActions').append(edit);}const b=el('button','Omów z DONĄ','primary');b.onclick=()=>{dialog.close();discuss(collection,id);};$('detailActions').append(b);dialog.showModal();
  }
  ...
  document.addEventListener('click',async e=>{const n=e.target.closest('[data-prompt],[data-tool],[data-detail],[data-discuss],[data-refresh],[data-chat],[data-live],[data-logout],[data-history-refresh]');if(!n)return;
