(function () {
  ...
  const labels = {today:'Dzisiaj',mail:'Poczta',approvals:'Do zatwierdzenia',inquiries:'Zapytania',offers:'Oferty',clients:'Klienci',calendar:'Kalendarz',files:'Pliki',marketing:'Marketing i WWW',tools:'Narzędzia',history:'Historia rozmowy',settings:'Ustawienia'};
  ...
  function render(){
    ...
    const v=state.view;document.title='DONA — '+labels[v];$('breadcrumbTitle').textContent=labels[v];$('pageTitle').textContent=labels[v];$('pageEyebrow').textContent=v==='today'?new Date().toLocaleDateString('pl-PL',{timeZone:'Europe/Warsaw',weekday:'long',day:'numeric',month:'long'}).toUpperCase():'TWOJA PRZESTRZEŃ';
    $('pageSubtitle').textContent=window.DonaBusiness?.descriptions[v]|| (v==='today'?(state.data?(pending().length?'Sprawdź przygotowane materiały i zaplanuj kolejny krok.':'Zobacz aktualne sprawy i wybierz, czym dziś zajmie się DONA.'):'Twoje sprawy w jednym miejscu.'):descriptions[v]||'Rozmowa, wiedza i potwierdzone wyniki Twojej Dony.');
    ...
    $('modeLabel').textContent=window.DonaTrace?.isWebinar?.()?(state.demo?'WEBINAR · DEMO':'WEBINAR · LIVE'):(state.demo?'DANE PRZYKŁADOWE':'DONA');
    const approvalNode=$('approvalCount');if(approvalNode){approvalNode.hidden=!state.data;approvalNode.textContent=String(pending().length+rows('socialPosts').filter(p=>p.status==='DRAFT').length);}
    ...
    window.DonaCommand.setSnapshot?.(state.data,state.demo);
    document.querySelectorAll('#mainNav details').forEach(d=>{if(d.querySelector('[aria-current=page]'))d.open=true;});
    if(window.DonaBusiness?.views[v]){window.DonaBusiness.render(v,state.data,state.demo);return;}
  ...
  function mailCard(r){
    const unread=/^(NEW|NOWY|NOWA|UNREAD)$/i.test(r.status||'');
    return '<article class="mail-card'+(unread?' is-unread':'')+'"><button class="mail-open" data-detail="mails" data-id="'+escape(r.id)+'" aria-label="Otwórz wiadomość: '+escape(r.title||r.id)+'"><span class="mail-unread" aria-hidden="true"></span><span class="mail-main"><span class="mail-heading"><strong>'+escape(r.sender||'Nieznany nadawca')+'</strong><time>'+escape(formatDate(r.date||r.createdAt,true))+'</time></span><span class="mail-account">'+escape(r.account||'Nieznana skrzynka')+'</span><b>'+escape(r.title||'(bez tematu)')+'</b><span class="mail-preview">'+escape(r.snippet||'Brak podglądu wiadomości.')+'</span></span></button><div class="mail-actions"><button class="secondary" data-detail="mails" data-id="'+escape(r.id)+'">Otwórz</button><button class="secondary" data-mail-reply="'+escape(r.id)+'">Odpowiedz</button><button class="secondary" data-discuss="mails" data-id="'+escape(r.id)+'">Omów z Doną</button><button class="text-button mail-delete" data-mail-trash="'+escape(r.id)+'">Usuń</button></div></article>';
  }
  function renderRecords(){const c=listConfig();const q=state.search.toLocaleLowerCase('pl');let list=rows(c.key).filter(r=>(!state.filter||r.status===state.filter)&&(!state.accountFilter||r.account===state.accountFilter)&&(!q||[r.name,r.title,r.email,r.sender,r.account,r.snippet,r.clientName,r.description].filter(Boolean).join(' ').toLocaleLowerCase('pl').includes(q)));
    if(!list.length){$('recordResults').innerHTML=empty('Brak pasujących wyników','Zmień frazę lub wybierz inny status.','search');return;}
    if(c.key==='mails'){
      list=list.slice().sort((a,b)=>(Date.parse(b.date||b.createdAt)||0)-(Date.parse(a.date||a.createdAt)||0)).slice(0,10);
      $('recordResults').innerHTML='<div class="mail-list" aria-label="10 najnowszych wiadomości">'+list.map(mailCard).join('')+'</div>';
      return;
    }
  ...
  const functionGroups=[
    {title:'Na co dzień',copy:'Rzeczy, których najczęściej potrzebujesz na telefonie.',items:[
      {id:'weather',name:'Pogoda',symbol:'☀',copy:'Prognoza tutaj lub dla wybranego miasta.'},
      {id:'maps',name:'Mapy i trasy',symbol:'⌖',copy:'Miejsca, dojazd i trasa przez wiele punktów.'},
      {id:'mail',name:'Poczta',symbol:'✉',copy:'Wiadomości, pilne sprawy i szkice odpowiedzi.'},
      {id:'calendar',name:'Kalendarz',symbol:'▦',copy:'Spotkania, terminy i planowanie zmian.'},
      {id:'drive',name:'Dysk Google',symbol:'□',copy:'Wyszukiwanie i odczyt Twoich plików.'},
      {id:'searchall',name:'Szukaj wszędzie',symbol:'⌕',copy:'Jedno wyszukiwanie w danych Dony.'}
    ]},
    {title:'Tworzenie i promocja',copy:'Materiały, strony oraz działania marketingowe.',items:[
      {id:'social',name:'Publikacje Facebook',symbol:'f',copy:'Szkice, poprawki i publikacja wybranego posta.'},
      {id:'media',name:'Rolki i materiały',symbol:'▶',copy:'Zlecenia produkcji oraz gotowe pliki.'},
      {id:'campaigns',name:'Kampanie',symbol:'↗',copy:'Plany kampanii, materiały i następne kroki.'},
      {id:'websites',name:'Strony WWW',symbol:'◎',copy:'Budowa, podgląd i poprawki stron.'},
      {id:'youtube',name:'YouTube → wiedza',symbol:'▷',copy:'Transkrypcja, podsumowanie i mapa myśli.'},
      {id:'researchhub',name:'Research',symbol:'⌕',copy:'Rynek, konkurencja i źródła.'}
    ]},
    {title:'Sprzedaż i firma',copy:'Klienci, pieniądze oraz dokumenty.',items:[
      {id:'sales',name:'Sprzedaż',symbol:'↗',copy:'Zapytania, oferty i kolejny kontakt.'},
      {id:'clients',name:'Klienci',symbol:'♙',copy:'Relacje, ustalenia i historia klienta.'},
      {id:'inquiries',name:'Zapytania',symbol:'?',copy:'Nowe kontakty i potrzeby klientów.'},
      {id:'offers',name:'Oferty',symbol:'zł',copy:'Przygotowane wyceny i ich status.'},
      {id:'finance',name:'Finanse',symbol:'zł',copy:'Koszty, płatności i abonamenty.'},
      {id:'files',name:'Pliki i analizy',symbol:'□',copy:'Wyniki pracy zapisane przez Donę.'},
      {id:'documents',name:'Dokumenty',symbol:'▤',copy:'Dokumenty firmowe i prawne.'},
      {id:'projects',name:'Projekty i zadania',symbol:'✓',copy:'Otwarte sprawy i ich przebieg.'}
    ]},
    {title:'Dona i system',copy:'Wiedza, połączenia i kontrola działania systemu.',items:[
      {id:'dona',name:'Rozmowa z Doną',symbol:'✦',copy:'Wydaj polecenie własnymi słowami.'},
      {id:'knowledge',name:'Wiedza Dony',symbol:'◇',copy:'Pamięć oraz zasady Twojej firmy.'},
      {id:'recipes',name:'Receptury',symbol:'♨',copy:'Bezpośrednie wejście do systemu kuchni.'},
      {id:'connections',name:'Połączenia',symbol:'⌁',copy:'Sprawdź, które usługi są podłączone.'},
      {id:'health',name:'Problemy',symbol:'!',copy:'Blokady wymagające rozwiązania.'},
      {id:'operations',name:'Praca agentów',symbol:'◉',copy:'Zobacz przebieg wykonywanych zadań.'},
      {id:'history',name:'Historia rozmowy',symbol:'↺',copy:'Wróć do wcześniejszych ustaleń.'},
      {id:'settings',name:'Ustawienia',symbol:'⚙',copy:'Głos, dostęp i ustawienia panelu.'}
    ]}
  ];
  const tools=[
  ...
  function renderTools(){
    const groups=functionGroups.map(group=>'<section class="function-group"><div class="function-group-head"><h2>'+escape(group.title)+'</h2><p>'+escape(group.copy)+'</p></div><div class="function-grid">'+group.items.map(item=>'<a class="function-card" href="#'+escape(item.id)+'" data-view="'+escape(item.id)+'"><span class="function-symbol" aria-hidden="true">'+escape(item.symbol)+'</span><span><strong>'+escape(item.name)+'</strong><small>'+escape(item.copy)+'</small></span><b aria-hidden="true">›</b></a>').join('')+'</div></section>').join('');
    const agents='<details class="agent-tools"><summary>Agenci specjalistyczni Dony</summary><p>Jeśli nie chcesz wybierać konkretnej funkcji, wskaż obszar i opisz rezultat.</p><div class="tool-grid">'+tools.map(t=>'<article class="tool-card">'+toolVisual(t.id)+'<div class="tool-card-content"><div class="tool-card-title"><span class="object-icon">'+icon(t.icon)+'</span><h3>'+escape(t.name)+'</h3></div><p>'+escape(t.copy)+'</p><button class="secondary" data-tool="'+escape(t.name)+'">Zleć zadanie '+icon('arrow')+'</button></div></article>').join('')+'</div></details>';
    $('viewContent').innerHTML='<section class="function-hero"><div><span class="biz-kicker">CENTRUM FUNKCJI</span><h2>Co chcesz teraz zrobić?</h2><p>Wybierz funkcję. Na telefonie do tego miejsca wrócisz zawsze przyciskiem „Narzędzia” na dole ekranu.</p></div><button class="primary" data-chat>Zapytaj Donę</button></section>'+groups+agents;
  }
  function renderSettings(){ $('viewContent').innerHTML='<section class="settings-section"><h2>Twoja przestrzeń</h2><div class="setting-row"><span>Firma<small>'+escape(state.demo?'Przykładowa firma':state.data?.context?.name||'Probatum')+'</small></span><span class="status-badge">Pilotaż właściciela</span></div><div class="setting-row"><span>Dostęp<small>Logowanie jest zapamiętane w tej przeglądarce, jeśli wybrano „Zapamiętaj mnie na tym urządzeniu”. Przycisk Wyloguj usuwa zapis.</small></span><button class="secondary" data-logout>Wyloguj</button></div><div class="setting-row"><span>Poprzedni widok<small>Możesz wrócić do wcześniejszego interfejsu rozmowy.</small></span><a class="secondary" href="../legacy.html">Otwórz ↗</a></div></section><section class="settings-section"><h2>Rozmowa i głos</h2><div class="setting-row"><span>Odczytywanie odpowiedzi<small>Włącz lub wyłącz głos DONY.</small></span><button class="secondary" id="settingsVoice">'+escape($('voiceBtn').textContent)+'</button></div><div class="setting-row"><span>Tempo głosu<small>Ustawienie używane przez głos przeglądarki.</small></span><button class="secondary" id="settingsRate">'+escape($('rateBtn').textContent)+'</button></div><div class="setting-row"><span>Rozmowa na żywo<small>Korzysta z mikrofonu i dotychczasowej integracji głosowej. Rozliczana według użycia.</small></span><button class="secondary" data-live>Rozmawiaj</button></div></section><section class="settings-section"><h2>Co jest połączone</h2><p>Czat, historia, załączniki, pamięć i głos korzystają z istniejących połączeń DONY. Widoki biznesowe pobierają zapisane dane. Samo otwarcie widoku nie uruchamia agentów ani wysyłek.</p><p>To pilotaż Twojej przestrzeni. Konta kolejnych firm i abonamenty nie są jeszcze uruchomione.</p></section>';
  ...
  function discuss(collection,id){const list=collection==='files'?(state.data?.files||[]):rows(collection),r=list.find(x=>x.id===id);if(!r)return;const subject=collection==='approvals'?'prośbę o zatwierdzenie':collection==='offers'?'ofertę':collection==='mails'?'wiadomość e-mail':'sprawę';draft('Sprawdź '+subject+' o identyfikatorze '+r.id+' ('+(r.title||r.name||'')+(collection==='mails'&&r.sender?' od '+r.sender:'')+'). Podsumuj aktualny stan i zaproponuj kolejny krok. Najpierw tylko odczyt — bez wysyłek i zmian.');}
  ...
  function showMailReply(mail){
    ...
    dialog.innerHTML='<form method="dialog" id="mailReplyForm"><div class="system-dialog-head"><div><span>SZYBKA ODPOWIEDŹ</span><h2>'+escape(mail.title||mail.id)+'</h2><p>Do: '+escape(mail.sender||'nieznany nadawca')+'. Napisz odpowiedź tutaj — nie musisz rozpoczynać rozmowy z Doną. Wiadomość trafi do istniejącej Skrzynki decyzji i nie zostanie wysłana przed zatwierdzeniem.</p></div><button type="button" class="icon-button" data-mail-dialog-close aria-label="Zamknij">×</button></div><div class="system-form-grid"><label class="is-wide"><span>Twoja odpowiedź</span><textarea name="reply" required maxlength="12000" placeholder="Napisz odpowiedź…"></textarea></label></div><div class="system-dialog-actions"><button type="button" class="secondary" data-mail-dialog-close>Anuluj</button><button type="submit" class="primary">Dodaj do wysłania</button></div></form>';
  ...
  // missed the intended one. Re-added 10.09 backed by a real, dedicated tool
  // (archiwizuj_wiadomosc, plan/apply/undo by Gmail message_id, fully reversible) instead of
  // the package-based action - see n8n workflow gaH6JsiPG1BwhcZA.
  function confirmMailArchive(mail){
    if(!mail.messageId){toast('Ta wiadomość nie ma jeszcze zapisanego identyfikatora Gmaila — odśwież dane i spróbuj ponownie.');return;}
    const dialog=ensureMailDialog();
    dialog.innerHTML='<div class="system-dialog-head"><div><span>POTWIERDŹ</span><h2>Zarchiwizować tę wiadomość?</h2><p>'+escape(mail.title||mail.id)+' (od '+escape(mail.sender||'nieznanego nadawcy')+') zniknie ze skrzynki odbiorczej Gmaila. Wiadomość nie zostanie skasowana i można ją przywrócić osobną prośbą do DONY — dotyczy WYŁĄCZNIE tej jednej wiadomości.</p></div><button type="button" class="icon-button" data-mail-dialog-close aria-label="Zamknij">×</button></div><div class="system-dialog-actions"><button type="button" class="secondary" data-mail-dialog-close>Anuluj</button><button type="button" class="primary" data-mail-archive-confirm>Tak, zarchiwizuj</button></div>';
    dialog.querySelectorAll('[data-mail-dialog-close]').forEach(b=>b.addEventListener('click',()=>dialog.close()));
    dialog.querySelector('[data-mail-archive-confirm]').addEventListener('click',async()=>{
      dialog.close();
      const prompt='Zarchiwizuj WYŁĄCZNIE tę jedną wiadomość: message_id="'+mail.messageId+'" (temat: "'+mail.title+'", od '+(mail.sender||'nieznanego nadawcy')+'). Użyj narzędzia archiwizuj_wiadomosc w trybie apply z dokładnie tym jednym message_id — NIE używaj akcji archiwizuj w narzędziu poczta (ta działa na całych paczkach maili, nie na jednej wiadomości). Nie ruszaj żadnej innej wiadomości. Potwierdź krótko wynik.';
      toast('DONA archiwizuje tę wiadomość — potwierdzenie zobaczysz w rozmowie.');
  ...
  // "Usuń" (10.09): mirror confirmMailArchive, but moves to Gmail Trash (usun_wiadomosc /
  // Email Safe Trash, plan/apply/undo) instead of just clearing INBOX - a real destructive-
  // feeling action needed its own dedicated tool too, not the package-based poczta action.
  function confirmMailTrash(mail){
    if(!mail.messageId){toast('Ta wiadomość nie ma jeszcze zapisanego identyfikatora Gmaila — odśwież dane i spróbuj ponownie.');return;}
    const dialog=ensureMailDialog();
    dialog.innerHTML='<div class="system-dialog-head"><div><span>POTWIERDŹ</span><h2>Usunąć tę wiadomość?</h2><p>'+escape(mail.title||mail.id)+' (od '+escape(mail.sender||'nieznanego nadawcy')+') trafi do Kosza Gmaila i zniknie stamtąd po ok. 30 dniach — tak jak w zwykłym Gmailu. Do tego czasu można ją przywrócić osobną prośbą do DONY. Dotyczy WYŁĄCZNIE tej jednej wiadomości.</p></div><button type="button" class="icon-button" data-mail-dialog-close aria-label="Zamknij">×</button></div><div class="system-dialog-actions"><button type="button" class="secondary" data-mail-dialog-close>Anuluj</button><button type="button" class="primary" data-mail-trash-confirm>Tak, usuń</button></div>';
    dialog.querySelectorAll('[data-mail-dialog-close]').forEach(b=>b.addEventListener('click',()=>dialog.close()));
    dialog.querySelector('[data-mail-trash-confirm]').addEventListener('click',async()=>{
      dialog.close();
      const prompt='Usuń WYŁĄCZNIE tę jedną wiadomość: message_id="'+mail.messageId+'" (temat: "'+mail.title+'", od '+(mail.sender||'nieznanego nadawcy')+'). Użyj narzędzia usun_wiadomosc w trybie apply z dokładnie tym jednym message_id — NIE używaj akcji archiwizuj ani żadnej innej akcji pakietowej w narzędziu poczta. Nie ruszaj żadnej innej wiadomości. Potwierdź krótko wynik.';
      toast('DONA przenosi tę wiadomość do kosza — potwierdzenie zobaczysz w rozmowie.');
      try{const result=await window.Dona.runBranch('poczta','Poczta',prompt);if(!result||result.ok===false)toast('Nie potwierdzono wyniku. Sprawdź rozmowę przed ponowieniem.');else if(state.view==='mail')loadData();}
      catch(err){toast('Nie potwierdzono wyniku. Sprawdź rozmowę przed ponowieniem.');}
    });
    dialog.showModal();
  ...
  document.addEventListener('click',async e=>{const n=e.target.closest('[data-prompt],[data-tool],[data-detail],[data-discuss],[data-mail-reply],[data-mail-trash],[data-refresh],[data-chat],[data-live],[data-logout],[data-history-refresh],[data-add-client]');if(!n)return;
    if(n.hasAttribute('data-prompt'))draft(n.dataset.prompt);else if(n.hasAttribute('data-tool')){window.Dona.branch(n.dataset.tool);openChat();}else if(n.hasAttribute('data-detail'))showDetail(n.dataset.detail,n.dataset.id);else if(n.hasAttribute('data-discuss'))discuss(n.dataset.discuss,n.dataset.id);else if(n.hasAttribute('data-mail-reply')){const mail=rows('mails').find(r=>r.id===n.dataset.mailReply);if(mail)showMailReply(mail);}else if(n.hasAttribute('data-mail-trash')){const mail=rows('mails').find(r=>r.id===n.dataset.mailTrash);if(mail)confirmMailTrash(mail);}else if(n.hasAttribute('data-refresh'))loadData();else if(n.hasAttribute('data-chat'))openChat();else if(n.hasAttribute('data-live'))$('golive').click();else if(n.hasAttribute('data-logout')){if(state.demo)location.href='./';else window.Dona.logout();}else if(n.hasAttribute('data-history-refresh')){if(state.demo)toast('Historia jest dostępna po zalogowaniu.');else{await window.Dona.refreshHistory();renderHistory();}}else if(n.hasAttribute('data-add-client')){if(state.demo)toast('To podgląd na przykładowych danych. Dodawanie klientów jest dostępne po zalogowaniu.');else showAddClient();}});
