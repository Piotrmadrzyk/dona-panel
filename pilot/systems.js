(function (root) {
  ...
  function renderResearch() {
    ...
    // Piotr (10.09): "co za przycisk 'kontynuuj'? co mam kontynuowac?" - a bare arrow icon
    // and one-word label didn't say what happens. Also raw topic (often the whole original
    // request, hundreds of characters) rendered as an unbroken wall-of-text heading.
    html += research.length ? '<div class="research-list">' + research.map(item => '<article><div class="research-card-top"><span>Analiza rynku</span><time>' + escape(date(item.updatedAt || item.createdAt)) + '</time></div><h4>' + escape(short(item.topic,140)) + '</h4><p>' + escape(short(item.summary,420)) + '</p><footer><span>' + number(item.highConfidenceClaims) + ' twierdzeń o wysokiej pewności</span><button class="link-button" data-system-action="research-continue" data-id="' + escape(item.id) + '" data-title="' + escape(item.topic) + '" title="Odśwież ten temat: DONA sprawdzi, co zmieniło się od ostatniego razu">Sprawdź co się zmieniło ' + icon('arrow') + '</button></footer></article>').join('') + '</div>' : empty('Brak zapisanych analiz', 'Uruchom pierwszy research i zachowaj jego wynik w pamięci.');
