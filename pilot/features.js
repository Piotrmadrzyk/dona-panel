...
let snapshot=null,demo=false,busy=false,lastWeather='',lastMap='',weatherAutoTried=false;
// Piotr mieszka i pracuje w Rzeszowie - to jest sensowny domyślny widok, dopóki sam nie
// sprawdzi innego miasta. Wtedy TO miasto staje się nowym domyślnym, aż do kolejnej zmiany.
const DEFAULT_WEATHER_PLACE='Rzeszów';
function savedWeatherPlace(){try{return localStorage.getItem('dona_weather_place')||DEFAULT_WEATHER_PLACE;}catch{return DEFAULT_WEATHER_PLACE;}}
function saveWeatherPlace(place){try{if(place)localStorage.setItem('dona_weather_place',place);}catch{}}
// Maszynka do Shortów to osobne, lokalne narzędzie (Node/Express, repo pm-pages) —
// panel go NIE hostuje, tylko osadza jako iframe. Dlatego adres jest ustawialny:
// domyślnie localhost (appka i przeglądarka na tym samym komputerze), z opcją
// wpisania innego adresu, gdyby narzędzie kiedyś trafiło na serwer.
const DEFAULT_MASZYNKA_URL='http://localhost:3000';
function savedMaszynkaUrl(){try{return localStorage.getItem('dona_maszynka_url')||DEFAULT_MASZYNKA_URL;}catch{return DEFAULT_MASZYNKA_URL;}}
function saveMaszynkaUrl(url){try{if(url)localStorage.setItem('dona_maszynka_url',url);}catch{}}
function renderMaszynka(){const url=savedMaszynkaUrl();return '<div class="feature-hero"><span class="eyebrow">MASZYNKA DO SHORTÓW</span><h2>Brief → sceny → lektor i render MP4, bez wychodzenia z panelu.</h2><p>Lokalne narzędzie (repo pm-pages, <code>tools/maszynka-do-szortow</code>): pionowe wideo do social media z tekstem, lektorem i efektami ElevenLabs (przez DONĘ, bez osobnego klucza API). Musi działać na TYM komputerze — uruchom <code>npm start</code> w jego folderze, panel wczyta je poniżej.</p></div><section class="feature-panel"><h2>Adres narzędzia</h2><form class="feature-form" id="maszynkaUrlForm"><input id="maszynkaUrlInput" type="url" required placeholder="http://localhost:3000" value="'+E(url)+'"><button class="primary" type="submit">Wczytaj</button></form><p class="provider-note">Domyślnie <code>http://localhost:3000</code>. Zmień tylko jeśli uruchomisz narzędzie na innym porcie albo przeniesiesz je na serwer.</p></section><div class="tool-frame-wrap"><iframe class="tool-frame" title="Maszynka do Shortów" src="'+E(url)+'"></iframe></div><p class="provider-note">Puste pole albo błąd wczytywania? Sprawdź, czy narzędzie jest uruchomione, albo <a href="'+E(url)+'" target="_blank" rel="noopener noreferrer">otwórz je w nowej karcie ↗</a>.</p>';}
...
// GA4 rows are keyed by domain (e.g. "probatum.pl"), not the internal site.id -
// match on hostname (stripped of a leading "www.") so it lines up with what the
// GA4 collection workflow actually writes to PM_statystyki_www.
function siteDomain(site){return new URL(site.url).hostname.replace(/^www\./i,'');}
function siteStatBadge(site){
 const stat=snapshot&&snapshot.wwwStats&&snapshot.wwwStats[siteDomain(site)];
 if(!stat||stat.odslony===null||stat.odslony===undefined)return '';
 const zakresLabel=({yesterday:'wczoraj',today:'dziś',last7days:'7 dni',last30days:'30 dni',lastCalendarWeek:'ub. tydzień',lastCalendarMonth:'ub. miesiąc'})[stat.zakres]||stat.zakres||'';
 return '<span class="site-stat-badge" title="Źródło: Google Analytics 4. Zebrano: '+E(new Date(stat.zebrano).toLocaleString('pl-PL',{timeZone:'Europe/Warsaw'}))+'">👁 '+E(Number(stat.odslony).toLocaleString('pl-PL'))+' odsłon'+(zakresLabel?' · '+E(zakresLabel):'')+'</span>';
}
function siteCard(site){return '<article class="site-card"><div class="site-preview"><div class="browser-bar"><i></i><i></i><i></i><span>'+E(new URL(site.url).hostname)+'</span></div><img class="site-shot" src="'+E(site.preview)+'" alt="'+E(site.previewAlt)+'" loading="lazy" decoding="async"><span class="site-preview-kind">'+E(site.kind)+'</span></div><div class="site-body"><div class="site-meta"><span>'+E(site.kind)+'</span><span>'+E(site.status)+'</span></div><h3>'+E(site.name)+'</h3><p>'+E(site.copy)+'</p>'+siteStatBadge(site)+'<div class="command-actions"><a class="secondary" href="'+E(site.url)+'" target="_blank" rel="noopener noreferrer">Otwórz ↗</a><button class="primary" data-site-edit="'+E(site.id)+'">Edytuj</button></div></div></article>';}
...
function mediaCard(record){
 ...
 return '<article class="media-result media-result-'+E(status.toLowerCase())+'"><div class="media-result-head"><span class="media-folder-icon" aria-hidden="true">▰</span><div><span class="eyebrow">FOLDER ANALIZY · '+E(mediaDate(record&&record.processedAt))+'</span><h3>'+E(record&&record.title||'Analiza filmu')+'</h3></div><span class="media-result-status">'+E(statusLabel)+'</span></div>'+
  ...
  '<div class="media-file-links">'+(folder?'<a class="primary" href="'+E(folder)+'" target="_blank" rel="noopener noreferrer">Otwórz folder w Google Drive ↗</a>':'')+files.map(file=>{const link=D(file&&file.url);return link?'<a class="secondary" href="'+E(link)+'" target="_blank" rel="noopener noreferrer">'+E(file.name||'Plik')+' ↗</a>':'';}).join('')+(source?'<a class="text-link" href="'+E(source)+'" target="_blank" rel="noopener noreferrer">Film źródłowy ↗</a>':'')+
  // Piotr (10.09): "skoro nieudana to albo daj mi przycisk ponow albo usun" - a FAILED card with
  // no way forward was a dead end sitting forever in the history. Retry re-runs the same source
  // URL through the normal analysis flow; there is no delete here yet (would need a new backend
  // tool to remove a PM_media_registry row - not built, do not claim otherwise).
  (status==='FAILED'&&source?'<button class="secondary" type="button" data-media-retry="'+E(source)+'">Ponów analizę</button>':'')+
  '</div></article>';
}
...
function renderWeather(){const place=savedWeatherPlace();return '<div class="feature-hero weather-hero"><span class="eyebrow">POGODA Z LOKALIZACJĄ URZĄDZENIA</span><h2>Zapytaj DONĘ, zanim wyjdziesz.</h2><p>„Jaka pogoda?” może użyć bieżącej lokalizacji dopiero po Twojej zgodzie. Współrzędne nie są zapisywane; do prognozy wysyłana jest tylko przybliżona lokalizacja.</p><div class="command-actions"><button class="primary" type="button" data-weather-current>Sprawdź pogodę u mnie</button></div></div><section class="feature-panel"><h2>Prognoza dla miejscowości</h2><form class="feature-form" id="weatherForm"><input id="weatherPlace" type="search" required maxlength="120" autocomplete="address-level2" placeholder="np. Rzeszów" value="'+E(place)+'"><button class="primary" type="submit">Sprawdź</button></form><p class="provider-note">Domyślnie: '+E(place)+'. Sprawdź inne miasto, a zapamiętam je jako nowe domyślne.</p><div class="feature-status" id="weatherStatus" role="status"></div><div class="feature-results weather-results" id="weatherResults">'+lastWeather+'</div><p class="provider-note">Dane pogodowe: <a href="https://open-meteo.com/" target="_blank" rel="noopener noreferrer">Open-Meteo</a>. Bezpłatne źródło użyte w pilotażu; przed komercyjnym SaaS-em wymaga właściwej licencji lub zamiany dostawcy.</p></section>';}

// Maps URLs retain the supplied order; split longer routes for mobile browsers.
const MULTIROUTE_MAX_STOPS=10;
function buildMultiRouteUrl(origin,stops){
  const clean=stops.map(s=>String(s||'').trim()).filter(Boolean);
  if(clean.length<1||clean.length>4||clean.some(s=>s.includes('|')))return '';
  const destination=clean[clean.length-1];
  const waypoints=clean.slice(0,-1);
  const params=new URLSearchParams();
  params.set('api','1');
  const originTrimmed=String(origin||'').trim();
  if(originTrimmed)params.set('origin',originTrimmed);
...
function buildMultiRouteLegs(origin,stops){
  const clean=stops.map(s=>String(s||'').trim()).filter(Boolean);
  if(!clean.length||clean.length>MULTIROUTE_MAX_STOPS)return [];
  const legs=[];let start=String(origin||'').trim();
  for(let i=0;i<clean.length;i+=4){
    const group=clean.slice(i,i+4),url=buildMultiRouteUrl(start,group);
    if(!url)return [];
    legs.push({url,origin:start,stops:group});start=group[group.length-1];
  }
  return legs;
}
function routeStopField(n){return '<input class="route-stop" type="text" maxlength="180" placeholder="Przystanek '+n+'" aria-label="Przystanek '+n+'">';}
function renderMaps(){return '<div class="feature-hero maps-hero"><span class="eyebrow">MAPY I TRASY</span><h2>Pokaż miejsce. Wyznacz drogę.</h2><p>DONA może otworzyć ten widok na polecenie głosowe. Mapa w panelu korzysta z OpenStreetMap, a przycisk trasy otwiera Mapy Google bez płatnego API.</p><div class="command-actions"><button class="primary" type="button" data-map-current>Pokaż, gdzie jestem</button></div></div><section class="feature-panel"><h2>Znajdź miejsce lub cel trasy</h2><form class="feature-form" id="mapForm"><input id="mapQuery" type="search" required maxlength="180" placeholder="np. Zamek Lubomirskich w Rzeszowie"><button class="primary" type="submit">Pokaż na mapie</button></form><div class="feature-status" id="mapStatus" role="status"></div><div class="map-results" id="mapResults">'+lastMap+'</div><p class="provider-note">Mapa i geokodowanie: © <a href="https://www.openstreetmap.org/copyright" target="_blank" rel="noopener noreferrer">OpenStreetMap contributors</a>. Trasa otwiera się w Mapach Google.</p></section><section class="feature-panel"><h2>Trasa przez wiele miejsc</h2><p>Wpisz do 10 miejsc w kolejności odwiedzin. Dłuższą trasę podzielimy na etapy po maksymalnie 4 miejsca, aby można ją było otworzyć także na telefonie. Każdy następny etap zaczyna się tam, gdzie kończy się poprzedni.</p><form class="feature-form multiroute-form" id="multiRouteForm"><input id="routeOrigin" type="text" maxlength="180" placeholder="Start (puste = Twoja obecna lokalizacja)" aria-label="Start trasy"><div class="route-stops" id="routeStops">'+routeStopField(1)+routeStopField(2)+'</div><div class="command-actions"><button type="button" class="secondary" id="routeAddStop">+ Dodaj przystanek</button></div><div class="feature-status" id="routeStatus" role="status"></div><button class="primary" type="submit">Przygotuj trasę</button></form><p class="provider-note">Przystanki pozostają w podanej kolejności. Ten widok nie optymalizuje ich kolejności. Po otwarciu Map Google sprawdź, czy wszystkie miejsca zostały rozpoznane.</p></section>';}
function render(view,data,isDemo){snapshot=data;demo=isDemo;const root=document.getElementById('viewContent');if(view==='marketing')root.innerHTML=renderMarketing();else if(view==='recipes')root.innerHTML=renderRecipes();else if(view==='youtube'){root.innerHTML=renderYouTube();paintYouTube();}else if(view==='drive'){root.innerHTML=renderDrive();paintDrive();}else if(view==='weather'){root.innerHTML=renderWeather();if(!lastWeather&&!weatherAutoTried){weatherAutoTried=true;weatherForPlace(savedWeatherPlace(),'current').catch(()=>{});}}else if(view==='maps')root.innerHTML=renderMaps();else if(view==='maszynka')root.innerHTML=renderMaszynka();}
...
document.addEventListener('submit',e=>{if(e.target.id==='youtubeForm'){e.preventDefault();analyzeYouTube();}else if(e.target.id==='driveForm'){e.preventDefault();searchDrive();}else if(e.target.id==='weatherForm'){e.preventDefault();weatherForPlace(document.getElementById('weatherPlace').value,'current').catch(()=>{});}else if(e.target.id==='mapForm'){e.preventDefault();showMap(document.getElementById('mapQuery').value).catch(()=>{});}else if(e.target.id==='maszynkaUrlForm'){e.preventDefault();const url=document.getElementById('maszynkaUrlInput').value.trim();if(url){saveMaszynkaUrl(url);render('maszynka',snapshot,demo);}}else if(e.target.id==='revisionForm'){e.preventDefault();submitRevision(e.target);}else if(e.target.id==='multiRouteForm'){e.preventDefault();const status=document.getElementById('routeStatus');const origin=document.getElementById('routeOrigin').value;const stops=[...document.querySelectorAll('.route-stop')].map(input=>input.value);const legs=buildMultiRouteLegs(origin,stops);if(!legs.length){if(status){status.className='feature-status is-error';status.textContent='Podaj od 1 do 10 miejsc. Skróć bardzo długie adresy i usuń znak |.';}return;}if(status){status.className='feature-status';status.innerHTML='<p>Otwieraj etapy po kolei. Kolejność miejsc pozostaje taka, jak wpisano.</p>'+legs.map((leg,i)=>'<p><a class="secondary" href="'+E(leg.url)+'" target="_blank" rel="noopener noreferrer">Otwórz '+(legs.length===1?'trasę':'etap '+(i+1))+' w Mapach Google ↗</a><br><small>'+E((leg.origin||'Bieżąca lokalizacja')+' → '+leg.stops.join(' → '))+'</small></p>').join('');}}});
document.addEventListener('click',async e=>{const edit=e.target.closest('[data-site-edit]'),close=e.target.closest('[data-revision-close]'),read=e.target.closest('[data-drive-read]');if(close&&!busy)dialog.close();if(read)readDrive(read);if(edit){const site=[...owned,...realizations].find(x=>x.id===edit.dataset.siteEdit);if(site)openRevision({branchId:'www',branchName:'WWW',subject:site.name+' ('+site.url+')',context:site.copy+'\nStatus: '+site.status});}if(e.target.closest('[data-weather-current]'))weatherForCurrentLocation('current').catch(()=>{});if(e.target.closest('[data-map-current]'))mapCurrentLocation().catch(()=>{});if(e.target.closest('[data-catalog-add]'))window.Dona.draft('Chcę dodać brakującą stronę lub realizację do katalogu. Najpierw poproś mnie o adres, zweryfikuj własność i status; niczego nie publikuj ani nie wdrażaj.');const retry=e.target.closest('[data-media-retry]');if(retry&&youtubeState.phase!=='working'){youtubeState.url=retry.dataset.mediaRetry;paintYouTube();analyzeYouTube();}if(e.target.closest('[data-recipe-chat]')){await window.Dona.newThread();window.Dona.draft('Chcę porozmawiać o systemie Receptury. Najpierw ustal, której receptury lub funkcji dotyczy temat; bez zmieniania danych.');}if(e.target.closest('#routeAddStop')){const list=document.getElementById('routeStops');if(list){const count=list.querySelectorAll('.route-stop').length;if(count<MULTIROUTE_MAX_STOPS){const wrapper=document.createElement('div');wrapper.innerHTML=routeStopField(count+1);list.appendChild(wrapper.firstChild);}}}});
window.DonaFeatures={views:{recipes:'Receptury',youtube:'YouTube → wiedza',drive:'Dysk Google',weather:'Pogoda',maps:'Mapy i trasy',marketing:'Strony i realizacje',maszynka:'Maszynka do Shortów'},descriptions:{recipes:'Twój istniejący system receptur.',youtube:'Transkrypcja, podsumowanie i mapa myśli z filmu.',drive:'Bezpieczne wyszukiwanie i odczyt plików z Google Drive.',weather:'Prognoza dla bieżącej lokalizacji lub wskazanej miejscowości.',maps:'Wyszukiwanie miejsc, trasa przez wiele przystanków i uruchamianie tras bez płatnego API.',marketing:'Wizualny katalog wszystkich stron, systemów i realizacji.',maszynka:'Generuj krótkie wideo (shorts) z briefu — lektor, efekty, render MP4.'},render,openRevision,weatherForCurrentLocation,weatherForPlace,showMap,mapCurrentLocation,mediaCard,buildMultiRouteUrl,buildMultiRouteLegs,sites:()=>[...owned,...realizations]};
})();
