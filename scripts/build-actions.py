...
def node(var,name,kind,params,version=None,**extra):
    versions={'webhook':2.1,'dataTable':1.1,'code':2,'if':2.3,'respondToWebhook':1.5,'executeWorkflow':1.2}
...
def string_gate(var,name,expression,value):return node(var,name,'if',{'conditions':{'options':{'caseSensitive':True,'leftValue':'','typeValidation':'strict','version':2},'conditions':[{'leftValue':expression,'rightValue':value,'operator':{'type':'string','operation':'equals'}}],'combinator':'and'}})
...
auth=(r/'backend/workspace-auth.js').read_text().replace("body.operation !== 'snapshot'","!['decision','save_memory','publish_social'].includes(body.operation)")
...
string_gate('isPublish','Is Publish',expr('kind'),'publish')
...
write('saveSocial','Save Social Decision','ML98cvIaIgZtd7u9',[cond('id',expr('rowId')),cond('post_key',expr('id')),cond('status',expr('previousStatus')),cond('caption_hash',expr('captionHash')),cond('caption',expr('caption')),cond('image_sha256',expr('imageHash')),cond('profile_key',expr('profileKey')),cond('fb_page_id',expr('pageId')),cond('source_url',expr('sourceUrl')),cond('image_url',expr('imageUrl'))],{'status':expr('status'),'approved_by':'Piotr','approved_at':expr('now'),'approved_hash':expr('approvedHash')})
write('saveApproval','Save Approval Decision','KGyAqpVVwhv7G0Ra',[cond('id',expr('rowId')),cond('tenant_id','PM'),cond('status','REQUESTED'),cond('payload_preview',expr('payloadPreview')),cond('expires_at',expr('expiresAt')),cond('action_type',expr('actionType')),cond('payload_hash',expr('payloadHash'))],{'status':expr('status'),'approved_by':'panel:piotr','approved_at':expr('now'),'decision_source':'dona_panel','evidence':'Owner reviewed exact content in authenticated panel','execution_status':expr('executionStatus')})
...
publish_schema=[{'id':k,'displayName':k,'type':'string','required':False,'display':True,'canBeUsedToMatch':True} for k in ['operation','post_key','profile_key','tenant_id','role','original_message','expected_content_hash']]
node('callPublish','Call FB Publish','executeWorkflow',{'source':'database','workflowId':{'__rl':True,'mode':'id','value':'j781hsS6dVKdWFxh'},'workflowInputs':{'mappingMode':'defineBelow','value':{'operation':'publish_now','post_key':expr('id'),'profile_key':expr('profileKey'),'tenant_id':'PM','role':'OWNER','original_message':'Publikuj teraz ten wskazany post.','expected_content_hash':expr('contentHash')},'schema':publish_schema},'options':{}},onError='continueErrorOutput')
node('interpretPublish','Interpret Publish Outcome','code',{'jsCode':(r/'backend/publish-outcome.js').read_text(),'mode':'runOnceForAllItems'})
node('publishRespond','Return Publish Result','respondToWebhook',{'respondWith':'json','responseBody':'={{ $json }}','options':{'responseCode':'={{ $json.statusCode || 200 }}','responseHeaders':headers}})
settings={'executionOrder':'v1','executionTimeout':60,'saveDataErrorExecution':'none','saveDataSuccessExecution':'none','saveExecutionProgress':False,'saveManualExecutions':False,'timezone':'Europe/Warsaw','callerPolicy':'workflowsFromSameOwner','availableInMCP':True}
