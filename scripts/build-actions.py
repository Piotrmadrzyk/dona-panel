...
write('saveSocial','Save Social Decision','ML98cvIaIgZtd7u9',[cond('id',expr('rowId')),cond('post_key',expr('id')),cond('status','DRAFT'),cond('caption_hash',expr('captionHash')),cond('caption',expr('caption')),cond('image_sha256',expr('imageHash'))],{'status':expr('status'),'approved_by':'Piotr','approved_at':expr('now'),'approved_hash':expr('approvedHash')})
write('saveApproval','Save Approval Decision','KGyAqpVVwhv7G0Ra',[cond('id',expr('rowId')),cond('tenant_id','PM'),cond('status','REQUESTED'),cond('payload_preview',expr('payloadPreview')),cond('expires_at',expr('expiresAt'))],{'status':expr('status'),'approved_by':'panel:piotr','approved_at':expr('now'),'decision_source':'dona_panel','evidence':'Owner reviewed exact content in authenticated panel'})
