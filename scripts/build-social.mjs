...
add('entry','Social Tool Input','executeWorkflowTrigger',{inputSource:'workflowInputs',workflowInputs:{values:['operation','post_key','profile_key','tenant_id','role','original_message','expected_content_hash'].map(name=>({name,type:'string'}))}});
...
code('auth','Authorize Social Request',"const r=$input.first().json;return [{json:{...r,operation:r.operation||'read',ok:r.tenant_id==='PM'&&r.role==='OWNER'&&['read','status','publish_approved','publish_now'].includes(r.operation||'read'),error:'UNAUTHORIZED_OR_INVALID_OPERATION'}}];");
...
update('claim','Claim Exact Approved Post',{approved_at:'={{ $json.approval.approved_at }}',approved_by:'={{ $json.approval.approved_by }}',approved_hash:'={{ $json.approval.approved_hash }}',caption_hash:'={{ $json.approval.caption_hash }}',claim_execution:'={{ $json.claim }}',review_notes:'={{ $json.approval.review_notes || \'\' }}',scheduled_date:'={{ $json.today }}',status:'PUBLISHING'},[
  filter('id','={{ $json.post.id }}'),filter('status','={{ $json.post.status }}'),filter('post_key','={{ $json.post.post_key }}'),filter('profile_key','={{ $json.post.profile_key }}'),filter('caption','={{ $json.post.caption }}'),filter('image_sha256','={{ $json.post.image_sha256 }}'),filter('image_url','={{ $json.post.image_url }}'),filter('source_url','={{ $json.post.source_url }}'),filter('fb_page_id','={{ $json.post.fb_page_id }}'),filter('updatedAt','={{ $json.post.updatedAt }}')]);
...
const resultValues=Object.fromEntries(['status','buffer_post_id','published_url','last_error','scheduled_date'].map(k=>[k,'={{ $json.'+k+' }}']));
resultValues.published_at='={{ $json.published_at || null }}';
...
lines.push("export default workflow('dona-facebook-buffer','DONA — Facebook: kolejka i publikacja przez Buffer',"+JSON.stringify({settings:{availableInMCP:true,callerPolicy:'workflowsFromAList',callerIds:'vE77e9dXD2e93jBW,mILGt8TyRMEf6NZg',executionOrder:'v1',executionTimeout:120,timezone:'Europe/Warsaw'}})+")\n"+
