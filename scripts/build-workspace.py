...
tables=[('config','Tenant Configuration','Njc9seD34Eeck9Vf','client_id'),('customers','Read Customers','WlE2p5ab7BLjhoSr','client_id'),('approvals','Read Approvals','KGyAqpVVwhv7G0Ra','tenant_id'),('leads','Read Leads','gAMd9Fouz9dffQaD','client_id'),('offers','Read Offers','2jCWFfufSfo7X9kr','client_id'),('meetings','Read Meetings','XfBmAVssaQ1dvOpE','client_id'),('tasks','Read Tasks','9lpZx2cRIk7edt8a','tenant_id'),('documents','Read Documents','ESx6r7mHmbKM4rj3','klient_id'),('events','Read Events','OUh2puv86Ip0YZLE','client_id'),('brain','Read Brain','XeJ2EyohsX0nIrgz','tenant_id'),('mail','Read Mail','VaKLh60KD8Wng7yO','tenant_id'),('mailSync','Read Mail Sync','Fa9C1UUUMUEJtfC1','tenant_id'),('projectMemory','Read Project Memory','OnVQpSBfjfn6Oqla','tenant_id'),('panelEvents','Read Panel Events','prNvnc22Kdu4GVQI','tenant_id'),('zoho','Read Zoho Calendar','lOnEaEQ8varOsmdt','tenant_id')]
for var,name,tid,field in tables:
    table(var,name,tid,field,"={{ $('Validate Access').first().json.tenantId }}",limit=51 if var in ['mail','mailSync','projectMemory','panelEvents','zoho'] else 251)
