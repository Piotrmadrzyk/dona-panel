...
const PREVIEW_ROOT='../pilot/assets/site-previews/';
const owned=[
...
// aby marnowal czas na jezdzenie od wschodu do zachodu i znow na wschod" - a single-point
// finder couldn't do this at all. Google's own free Directions URL understands multiple
// waypoints and can reorder them for the shortest route (optimize:true) - no paid API, no
// geocoding needed on our side, Maps resolves place names itself once opened.
const MULTIROUTE_MAX_STOPS=10;
function buildMultiRouteUrl(origin,stops,optimize){
  ...
  if(waypoints.length)params.set('waypoints',(optimize?'optimize:true|':'')+waypoints.join('|'));
...
document.addEventListener('submit',e=>{if(e.target.id==='youtubeForm'){e.preventDefault();analyzeYouTube();}else if(e.target.id==='driveForm'){e.preventDefault();searchDrive();}else if(e.target.id==='weatherForm'){e.preventDefault();weatherForPlace(document.getElementById('weatherPlace').value,'current').catch(()=>{});}else if(e.target.id==='mapForm'){e.preventDefault();showMap(document.getElementById('mapQuery').value).catch(()=>{});}else if(e.target.id==='revisionForm'){e.preventDefault();submitRevision(e.target);}else if(e.target.id==='multiRouteForm'){e.preventDefault();const status=document.getElementById('routeStatus');const origin=document.getElementById('routeOrigin').value;const stops=[...document.querySelectorAll('.route-stop')].map(input=>input.value);const optimize=document.getElementById('routeOptimize').checked;const url=buildMultiRouteUrl(origin,stops,optimize);if(!url){if(status){status.className='feature-status is-error';status.textContent='Podaj przynajmniej jeden przystanek.';}return;}if(status){status.className='feature-status';status.textContent='Otwieram trasę w Mapach Google…';}window.open(url,'_blank','noopener');}});
