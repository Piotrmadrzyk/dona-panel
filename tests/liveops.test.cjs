...
test('Live Ops frontend is wired to a read-only endpoint and polls only in its active view',()=>{
  ...
  assert.match(html,/liveops\.css\?v=1\.0\.0/);
  assert.match(html,/liveops\.js\?v=1\.0\.0/);
  assert.match(html,/panel-actions\.js\?v=5\.2\.0/);
  assert.match(html,/systems\.js\?v=6\.0\.0/);
  assert.match(html,/workspace\.js\?v=6\.0\.1/);
