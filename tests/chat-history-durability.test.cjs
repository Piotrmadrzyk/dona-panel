...
test('Dona\'s answer is no longer saved client-side after a fetch succeeds - the server now owns that write',()=>{
  ...
  // This applies to main chat, branch dispatch and the Realtime tool result. Spoken Realtime
  // summaries remain separate conversation turns and are still saved from transcript events.
  assert.doesNotMatch(chat,/add\('dona',ans\);zapiszHist\('dona',ans\)/);
  assert.doesNotMatch(chat,/zapiszHist\('dona','\['\+selected\.name/);
  assert.doesNotMatch(chat,/add\('dona',answer\);zapiszHist\('dona',answer\)/);
  assert.match(chat,/Panel Dona Apex now saves Dona's answer to history/);
  assert.match(chat,/Panel Dona Apex zapisuje wynik narzędzia po stronie serwera/);
});
