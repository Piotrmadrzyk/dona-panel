...
function processState(value) { const state=parse(value); return {settled:simpleList(state.ustalone),waiting:simpleList(state.czeka_na_ciebie),blocked:simpleList(state.zablokowane)}; }
...
const tasks=read('Read Tasks','tenant_id','tasks').map(r=>({id:text(r.zadanie_id),title:text(r.tytul),description:text(r.opis),date:text(r.due_at||r.termin),status:text(r.status),clientId:text(r.customer_id||r.klient_id),clientName:clientName(r.customer_id||r.klient_id),projectId:text(r.projekt_id),priority:text(r.explicit_priority||r.priorytet),priorityScore:number(r.priority_score),priorityReasons:text(r.priority_reasons,2000),owner:text(r.wlasciciel),sourceType:text(r.source_type),acceptanceCriteria:text(r.kryteria_odbioru,2000),result:text(r.wynik,4000),completedAt:text(r.completed_at),createdAt:text(r.utworzono||r.createdAt)}));
const processes=read('Read Processes','tenant_id','processes').map(r=>({id:text(r.proces_id),title:text(r.opis||r.typ||r.proces_id),type:text(r.typ),description:text(r.opis,4000),currentStep:text(r.krok_obecny,4000),status:text(r.status),state:processState(r.stan_json),requestedBy:text(r.requested_by),result:text(r.wynik,5000),createdAt:text(r.utworzono||r.createdAt),updatedAt:text(r.ostatnia_aktywnosc||r.updatedAt)}));
...
const researches=read('Read Research','client_id','researches').map(r=>({id:text(r.temat_klucz),topic:text(r.temat||r.temat_klucz),summary:text(r.podsumowanie,7000),mode:text(r.tryb),projectId:text(r.projekt_id),clientName:clientName(r.klient_id),highConfidenceClaims:number(r.liczba_twierdzen_wysoka_pewnosc),createdAt:text(r.createdAt),updatedAt:text(r.updatedAt)})).filter(r=>r.id);
const competitorObservations=read('Read Competitor Observations','client_id','competitorObservations').map(r=>({id:text(r.obserwacja_id),competitor:text(r.konkurent),area:text(r.obszar),summary:text(r.skrot,4000),whatChanged:text(r.co_sie_zmienilo,4000),changed:r.zmiana===true,weight:text(r.waga),date:text(r.czas||r.createdAt)})).filter(r=>r.id);
