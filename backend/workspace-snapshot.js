...
function read(name, tenantField, key) {
  ...
  const limit=['mails','mailboxSync','permanentMemory','workHistory','calendar'].includes(key)?50:cap;
...
for(const collection of [leads,offers,clients,meetings,tasks,files,activity])for(const item of collection)if(!item.brandId)item.brandId='';
return [{json:{ok:true,generatedAt:new Date().toISOString(),context:{tenantId:tenant,name:text(configs[0].nazwa)||'Probatum',userName:'Piotr',role:'OWNER'},approvals,leads,offers,clients,meetings,tasks,files,activity,memory,permanentMemory,socialProfiles,socialPosts,connections,mails,mailboxSync,workHistory,calendar:{provider:'zoho',status:zoho.length?'CONFIGURED':'NOT_CONNECTED',calendars:zoho},meta:{limit:cap,truncated,mode:'owner_pilot',readOnly:true}}}];
