...
const memory=read('Read Brain','tenant_id','memory').map(r=>({id:text(r.wpis_id),text:text(r.tresc,6000),type:text(r.typ),status:text(r.status),tags:text(r.tagi),source:text(r.zrodlo_kanal)||'PM_drugi_mozg',sourceRef:text(r.zrodlo_ref),updatedAt:text(r.aktualizacja||r.updatedAt)}));
...
{id:'calendar',name:'Kalendarz',status:configs[0].calendar_id?'CONFIGURED':'UNKNOWN',detail:configs[0].calendar_id?'Wybrany kalendarz: '+text(configs[0].calendar_id)+'. Dostęp do Google nie był sprawdzany w tym odczycie.':'Brak identyfikatora kalendarza w konfiguracji.',source:'PM_tenant_config'},
...
...socialProfiles.map(p=>({id:p.id,name:p.name+' · Facebook',status:p.connection==='META_NOT_CONNECTED'?'NOT_CONNECTED':p.enabled?'CONFIGURED':'PAUSED',detail:p.connection==='META_NOT_CONNECTED'?'Brak połączenia z Meta. Posty nie mogą się publikować.':p.enabled?'Publikacja włączona w konfiguracji. Ten odczyt nie sprawdza tokenu ani wykonania harmonogramu.':'Publikacja wyłączona.',source:p.source}))];
return [{json:{ok:true,generatedAt:new Date().toISOString(),context:{tenantId:tenant,name:text(configs[0].nazwa)||'Probatum',userName:'Piotr',role:'OWNER'},approvals,leads,offers,clients,meetings,tasks,files,activity,memory,socialProfiles,socialPosts,connections,meta:{limit:cap,truncated,mode:'owner_pilot',readOnly:true}}}];
